...
def main() -> None:
    ...
    for index in range(total_files):
        ...
        with torch.no_grad():
            ...
            # Sanitize potential NaNs/Infs from quantized weights and clamp to valid range
            sr_y_tensor = torch.nan_to_num(sr_y_tensor, nan=0.0, posinf=1.0, neginf=0.0)
            sr_y_tensor = torch.clamp(sr_y_tensor, 0.0, 1.0)

        # Save image
        # Important: clone to avoid in-place squeeze_ inside tensor_to_image modifying sr_y_tensor
        sr_y_image = imgproc.tensor_to_image(sr_y_tensor.clone(), range_norm=False, half=False)
