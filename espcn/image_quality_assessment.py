...
def _psnr_torch(raw_tensor: torch.Tensor, dst_tensor: torch.Tensor, crop_border: int,
    ...
    if only_test_y_channel:
        # If tensors are already single-channel (Y), skip conversion
        if raw_tensor.shape[1] == 3 and dst_tensor.shape[1] == 3:
    ...
    # Use float32 for broad device support (MPS has no float64)
